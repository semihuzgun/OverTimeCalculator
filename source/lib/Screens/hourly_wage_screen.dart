...
class HourlyWageScreen extends StatefulWidget {
  ...
  @override
  State<HourlyWageScreen> createState() => _HourlyWageScreenState();
}

class _HourlyWageScreenState extends State<HourlyWageScreen> {
  ...
  static const double _monthlyHourBase = 225;
  ...
  Future<void> _loadLastInputs() async {
    ...
    final last = await StorageService.loadLastInputsHourly();
    if (!mounted || last == null) return;

    final h = last['hourly'] ?? 0;
    if (h > 0) _hourlyController.text = _formatSaved(h);
  ...
  void _showSaveToCumulativeDialog() {
    if (totalGrossResult == null || totalGrossResult! <= 0) return;

    final totalBrut = totalGrossResult!;
  ...
  void _calculate() {
    final hourly = double.tryParse(_hourlyController.text) ?? 0;
    final cumulativeFromUI = double.tryParse(_cumulativeController.text) ?? 0;

    final h1 = double.tryParse(_hours1Controller.text) ?? 0;
    final r1 = double.tryParse(_rate1Controller.text) ?? 150;
    final h2 = double.tryParse(_hours2Controller.text) ?? 0;
    final r2 = double.tryParse(_rate2Controller.text) ?? 200;

    if (hourly <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Saatlik brüt ücret 0\'dan büyük olmalı.'),
        ),
      );
      return;
    }

    final baseGross = hourly * _monthlyHourBase;
    final overtimeList = <Map<String, double>>[
      {"hours": h1, "rate": r1},
      {"hours": h2, "rate": r2},
    ];

    final overtimeGross = PayrollCalculator.calculateOvertimeDynamic(
      baseGross,
      overtimeList,
    );

    final net = PayrollCalculator.calculateNet(
    ...
    setState(() {
      grossOvertimeResult = overtimeGross;
      totalGrossResult = baseGross + overtimeGross;
      netResult = net;
    });

    StorageService.saveLastInputsHourly(
      hourly: hourly,
  ...
  Widget build(BuildContext context) {
    final currency = NumberFormat.currency(locale: 'tr_TR', symbol: '₺');
    final displayMonthName =
        DateFormat.MMMM('tr_TR').format(DateTime(2026, selectedMonth));

    return Scaffold(
      appBar: AppBar(title: const Text("Saatlik Brüt Ücret ile Hesaplama")),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          children: [
            _buildInput("Saatlik Brüt Ücret (₺)", _hourlyController),
